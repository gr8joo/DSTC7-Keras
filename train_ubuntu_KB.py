...
from models.memLstm2_ubuntu_KB import memLstm_custom_model
...
def main():
    ...
    model.load_weights('/tmp/weights/5hops_0_0_1832_KB_noflag.h5')
    ...
    context_attention,\
    responses_attention,\
    kb_attention = model.predict(predict_X, batch_size=50, verbose=1)
    predict_target = np.argmax(predict_Y, axis=-1)
    print(predict_Y.shape)
    print(context_attention.shape)
    print(responses_attention.shape)
    print(kb_attention.shape)

    ### TODO: Visualize KB attention as well ###
