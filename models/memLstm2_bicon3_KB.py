...
from keras.layers import Embedding, Reshape, Lambda, Permute, NonMasking, Add, Dot, Multiply, Concatenate, Maximum


def memLstm_KB_model(hparams, context, context_speaker, utterances, knowledges):
    ...
    print("knowledge_shape: ", knowledges._keras_shape)
    ...
    embedding_knowledge_layer = Embedding(input_dim=hparams.vocab_size,
                            output_dim=hparams.memn2n_embedding_dim,
                            weights=[embeddings_W],
                            input_length=hparams.max_knowledge_len,
                            mask_zero=True,
                            trainable=False)
    ...
    # Define LSTM Knowledge encoder
    LSTM_K = LSTM(hparams.memn2n_rnn_dim,
                        input_shape=(hparams.max_knowledge_len, hparams.memn2n_embedding_dim),
                        unit_forget_bias=True,
                        return_state=False,
                        return_sequences=False)
    ...
    # Define Max layer
    max_layer = Lambda(lambda x: K.max(x, axis=-1, keepdims=False))
    ...
    # Knowledge Embedding: (BATCH_SIZE(?) x NUM_KNOWLEDGE_OPTIONS x KNOWLEDGE_LEN x EMBEDDING_DIM)
    knowledges_embedded = TimeDistributed(embedding_knowledge_layer,
                                            input_shape=(hparams.num_knowledge_options,
                                                        hparams.max_knowledge_len))(utterances)
    print("Utterances_embedded: ", utterances_embedded.shape)
    print("Utterances_embedded (history): ", utterances_embedded._keras_history, '\n')
    ...
    # Encode context (Output shape: BATCH_SIZE(?) x NUM_KNOWLEDGE_OPTIONS x RNN_DIM)
    all_knowledges_encoded_K = TimeDistributed(LSTM_K,
                                                input_shape=(hparams.num_knowledge_options,
                                                            hparams.max_knowledge_len,
                                                            hparams.memn2n_embedding_dim))(knowledges_embedded)
    print("all_utterances_encoded_C: ", all_knowledges_encoded_K.shape)
    print("all_utterances_encoded_C: (history)", all_knowledges_encoded_K._keras_history, '\n')


    # Score utterances by knowledge
    # (Output shape: BATCH_SIZE(?) x NUM_UTTERANCE_OPTIONS(100) x NUM_KNOWLEDGE_OPTIONS))
    knowledge_scores = Dot(axes=[2,2])([all_utterances_encoded_B,
                                        all_knowledges_encoded_K])
    # knowledge_scores = Maximum()([knowledge_scores])
    knowledge_scores = max_layer(knowledge_scores)
    print("knowledge_scores: ", knowledge_scores._keras_shape)
    print("knowledge_scores: (history)", knowledge_scores._keras_history, '\n')
    ...
    for i in range(hparams.hops):
        ...
        if i < hparams.hops-1:
            ...
            logits = Add()([logits, knowledge_scores])
