...
def memLstm_custom_model(hparams,
    ...
    custom_repeat_layer2= Lambda(lambda x: K.repeat_elements(x,
                                                            hparams.num_utterance_options+hparams.num_kb_options,
                                                            1))
    ...
    # Encode Knowledges K: (BATCH_SIZE(?) x NUM_OPTIONS(100) x RNN_DIM)
    all_kb_encoded_K = TimeDistributed(LSTM_K,
                                        input_shape=(hparams.num_kb_options,
                                                    hparams.max_kb_len,
                                                    hparams.memn2n_embedding_dim))(kb_embedded)
    all_kb_encoded_K = TimeDistributed(Matrix_utterances,
                                                input_shape=(hparams.num_kb_options,
    ...
    for i in range(hparams.hops):
        ...
        attention_Forward_uttr = Dot(axes=[2,2])([prof_aug_utterances_encoded_B,
                                                    prof_aug_context_encoded_Forward])
        attention_Forward_kb = Dot(axes=[2,2])([all_kb_encoded_K,
                                                    all_context_encoded_Forward])
        attention_Forward = Concatenate(axis=1)([attention_Forward_uttr,
                                                    attention_Forward_kb])
        ...
        all_utterances_encoded_B = Add()([GetUtterancesTensor(weighted_sum_Forward),
                                            all_utterances_encoded_B])
        all_kb_encoded_K =  Add()([GetKbTensor(weighted_sum_Forward),
                                            all_kb_encoded_K])
        ...
        attention_Backward_uttr = Dot(axes=[2,2])([prof_aug_utterances_encoded_B,
                                                    prof_aug_context_encoded_Backward])
        attention_Backward_kb = Dot(axes=[2,2])([all_kb_encoded_K,
                                                    all_context_encoded_Backward])
        attention_Backward = Concatenate(axis=1)([attention_Backward_uttr,
                                                    attention_Backward_kb])
        ...
        # (Output shape: ? x NUM_OPTIONS(100) x RNN_DIM)
        all_utterances_encoded_B = Add()([GetUtterancesTensor(weighted_sum_Backward),
                                            all_utterances_encoded_B])
        all_kb_encoded_K =  Add()([GetKbTensor(weighted_sum_Backward),
                                            all_kb_encoded_K])
        ...
        if i < hparams.hops-1:
            ...
            ########## Wighted Sum between context and external knowledge ##########
