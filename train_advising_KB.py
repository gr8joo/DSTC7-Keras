...
from models.memLstm2_advising_KB import memLstm_custom_model
...
def main():
    ...
    context_attention,\
    responses_attention,\
    kb_attention = memLstm_custom_model(hparams,
    ...
    valid_Y = [valid_target, np.zeros((500,1,1),dtype='i4'),
    ...
    for i in range(10):
        ...
        for j in range(10):
            ...
            train_X = [np.take(train_context, sub_idx, axis=0), np.take(train_context_mask, sub_idx, axis=0),\
                        np.take(train_options, sub_idx, axis=0),
                        np.take(train_context_profile_flag, sub_idx, axis=0), np.take(train_options_profile_flag, sub_idx, axis=0),
                        np.take(train_kb, sub_idx, axis=0), np.take(train_kb_flag, sub_idx, axis=0), np.take(train_kb_mask, sub_idx, axis=0)]
    ...
    model.load_weights('/tmp/weights/1hops_1_6_900_5amp_advising.h5')
